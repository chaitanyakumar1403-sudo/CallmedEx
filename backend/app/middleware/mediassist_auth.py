...
The HMAC scheme here exactly mirrors `MediAssistClient._sign` in
app/integrations/mediassist_client.py — both directions share one secret,
`settings.MEDIASSIST_HMAC_SECRET`, so a divergence here would silently break
inbound verification without ever touching the outbound path.
"""
import hashlib
import hmac
import logging
...
async def verify_mediassist_signature(request: Request) -> None:
    ...
    expected_digest = hmac.new(
        ...
        f"{timestamp_header}.".encode() + body,
        hashlib.sha256,
    ).hexdigest()
